...
class gene_loc():
    """Class that records location information for gene"""
    def __init__(self, name, chrom, min_coord, max_coord, strand, introns, exons):
        ...
        def get_cdna_and_exon_locs(dna, exons):
            ...
            exons = [exon - normalizer for exon in exons]
            cdna = ''
            exon_juncs = []
            intron_sum = 0
...
open('genes_with_errors','w') as genes_with_errors_f, open('primers.gff','w') as gff_f:
    ...
    for idx, gene in enumerate(coord,1):
        ...
        if 'PRIMER_RIGHT_0' not in goi:
            ...
            continue
        temp.append(','.join([gene.name+'-q' + f_primer_map[gene.strand], goi['PRIMER_LEFT_0_SEQUENCE'], str(goi['PRIMER_LEFT_0_PENALTY']), str(goi['PRIMER_PAIR_0_PRODUCT_SIZE'])])+'\n')
        temp.append(','.join([gene.name+'-q'+ r_primer_map[gene.strand], goi['PRIMER_RIGHT_0_SEQUENCE'], str(goi['PRIMER_RIGHT_0_PENALTY']), str(goi['PRIMER_PAIR_0_PRODUCT_SIZE'])])+'\n')
    temp.sort()
